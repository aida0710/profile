//statusMessage
export const statusMessages: () => string[] = (): string[] => {
    return [
        '高校生 普通科 3年 Frontend/Backend Engineer',
        '現在は、Next.jsを使用したWebアプリやRustを用いた自作言語の開発を行っています。',
        '以前はPHPを使用したマインクラフトサーバの開発、運用、保守を行っていました。',
    ];
};

//skills
export const programmingLanguages: () => string[] = () => {
    return ['PHP 8.*', 'Python 3.*', 'Excel VBA', 'HTML 5', 'CSS 3', 'TypeScript', 'JavaScript', 'Rust'];
};

export const webRelated: () => string[] = (): string[] => {
    return ['React', 'Next.js', 'ChakraUI', 'TailwindCSS', 'Shadcn/ui'];
};

export const databases: () => string[] = (): string[] => {
    return ['MySQL 8', 'SQLite'];
};

export const amazonWebServices: () => string[] = (): string[] => {
    return ['EC2', 'RDS', 'S3'];
};

export const qualifications: () => string[] = (): string[] => {
    return ['ITパスポート R5 May', '普通自動車運転免許 R5 Aug'];
};

export const awards: () => string[] = (): string[] => {
    return ['東京大学 産学協創推進本部主催 100program 4期 優秀アプリ賞 受賞'];
};

export const others: () => string[] = (): string[] => {
    return ['Git', 'Nginx', 'Apache', 'Ubuntu 20.04', 'Node.js 18~21'];
};
