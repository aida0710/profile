  ...
  return (
    <aside className="fixed left-0 top-0 z-40 hidden h-screen w-60 flex-col border-r border-warm-border bg-warm-bg sm:flex">
      ...
      <div className="flex flex-col items-center px-6 pb-6 pt-8">
        ...
        <p className="text-sm text-warm-subtext">{t(locale, 'common.jobRole')}</p>
      </div>
          ...
          {NAVIGATION_ITEMS.map((item) => {
            ...
            return (
              <li key={item.path}>
                ...
                >
                  <span className={active ? 'text-warm-accent' : 'text-warm-subtext group-hover:text-warm-text'}>
                    {item.icon}
                  </span>
      ...
      <div className="flex items-center justify-between border-t border-warm-border px-6 py-4">
        <Link
          aria-label={t(locale, 'common.githubRepo')}
